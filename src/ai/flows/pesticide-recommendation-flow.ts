...
const RecommendedPesticideSchema = z.object({
  ...
  dosage: z.string().describe("Recommended dosage per unit area (e.g., ml/liter or kg/hectare)."),
...
3.  **Pesticide Recommendations**: If chemical pesticides are warranted (or as a later option if non-pesticide methods are insufficient):
    ...
    - For each pesticide: Name, Active Ingredient, Dosage, Application Timing, Application Method, and Waiting Period (if applicable).
...
Structure your output according to the 'PesticideRecommendationOutput' schema.
